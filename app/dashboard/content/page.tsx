...
// We define the structure of a "Post" so the computer doesn't get confused
interface Post {
  image_url: string;
  headline: string;
  body_content: string;
...
export default function ContentDashboard({ initialPosts = [] }: { initialPosts?: Post[] }) {
  // We set a default empty list [] so "any" type errors disappear
  ...
  return (
    <div className="p-10 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-10">
        ...
        <button onClick={scheduleAll} className="bg-blue-600 text-white px-8 py-3 rounded-full font-bold hover:bg-blue-700 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {posts && posts.length > 0 ? (
          ...
          <div className="col-span-full text-center py-20 text-gray-400">
            No content generated yet. Use the CMO Brain to start.
          </div>
        )}
      </div>
    </div>
