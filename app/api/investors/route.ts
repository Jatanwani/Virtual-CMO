...
function inferSectors(profile: any): string[] {
  ...
  const map: Record<string,string[]> = {
    'AI/ML': ['ai','machine learning','llm','gpt','nlp','artificial intelligence'],
...
async function calculateMatches(supabase: any, userId: string, profile: any) {
  ...
  const matches = investors.map((inv: any) => {
    ...
    if (overlap.length > 0) { score += Math.min(35, overlap.length*15); reasons.push(`${overlap.slice(0,2).join(' & ')} focus`) }
    ...
    else if (!isIndia && (geo.includes('global')||geo.includes('us')||geo.includes('worldwide'))) { score += 20; reasons.push('Global mandate') }
    else if (geo.includes('global')) { score += 10 }
    if (inv.check_size_max > 0) { score += 20; reasons.push('Active investor') }
    else score += 10
    return { user_id: userId, investor_id: inv.id, score: Math.min(score, 100), reasons: reasons.slice(0,3), calculated_at: new Date().toISOString() }
  })
...
export async function GET(request: Request) {
  try {
    ...
    // Get REAL type counts from ALL investors (not filtered)
    ...
    const { data: investors, count: filteredCount } = await query
    ...
    return NextResponse.json({
      investors: investors || [],
      total: typeFilter || sectorFilter || search ? (filteredCount || 0) : totalCount,
      page,
      totalPages: Math.ceil((typeFilter || sectorFilter || search ? (filteredCount || 0) : totalCount) / limit),
      typeCounts,
